    ...
    const info = {
        ...
        parameters: {
          ...
          N_Sticks_Selected: {
            ...
            pretty_name: 'Number of Sticks Selected',
            default: 5,
          ...
          automatic_selection: {
            type: jspsych.ParameterType.BOOL,
            pretty_name: 'Automatic Selection',
            default: true,
            description: 'Whether the advocate will automatically select the sticks for the judge'
          },
          judge_guess_on_same_screen: {
            type: jspsych.ParameterType.BOOL,
            pretty_name: 'Judge Guess On Same Screen',
            default: true,
            description: 'Whether the judge will enter their guess on the same screen as the sticks'    
          },
          trial_number: {
            type: jspsych.ParameterType.INT,
...
class StickTrialPlugin {
    ...
    trial(display_element, trial) {
       ...
       var stage_num = 1;
        ...
        function showAlertAndMaintainFullscreen(text) {
            alert(text);
        ...
        if (trial.judge_condition){
            ...
            // judge pre-amble should say in bold the trial number as "Round trial_number"
            var judge_preamble = `<p><strong>Round ${trial.trial_number} of ${trial.total_trials}</strong></p>`;

            if (trial.automatic_selection){
                if (trial.advocate_goal == "high"){
                    judge_preamble += `${trial.N_Sticks_Drawn} new sticks were drawn. Here are the ${trial.N_Sticks_Selected} tallest.`
                }
                else if (trial.advocate_goal == "low"){
                    judge_preamble = `${trial.N_Sticks_Drawn} new sticks were drawn. Here are the ${trial.N_Sticks_Selected} shortest.`
                }
                else if (trial.advocate_goal == "random"){
                    judge_preamble = `${trial.N_Sticks_Drawn} new sticks were drawn. Here are ${trial.N_Sticks_Selected} that have been randomly selected.`
                }
            }else{
            ...
            if (trial.judge_guess_on_same_screen){
                stage_num = 4;

                // add code for judge guess here
                var guess_preamble = `<p> Please enter a guess for the average stick length in the original pile of ${trial.N_Sticks_Drawn} sticks. <\p>`;

                // add guess_preamble to a new section below the sticks
                document.getElementById('sticks-container').innerHTML += guess_preamble + '<div id="guess-container">' +
                '<label for="stick-average-guess"></label>' +
                '<input type="text" id="stick-average-guess" name="stick-average-guess">' +
                '</div>';

                // add guess_preamble within stick-container, but below the sticks. place the guess input below this
            ...
            if (trial.multi_trial){
                var instruc_text = `<p>For this game, you goal is for the judge to make as ${advocate_goal} a guess as possible. <\p> <p>Sticks are ordered with smallest on the left and largest on the right. <\p> <p>Please select ${trial.number_sticks_selected} sticks to present to the judge.</p>`;
            }else{
                var instruc_text = `<p>Your goal is for the judge to make as ${advocate_goal} a guess as possible.<\p> <p>Sticks are ordered with smallest on the left and largest on the right. <\p> <p><strong>Please select ${trial.number_sticks_selected} sticks to present to the judge.</strong></p>`;
            }
        ...
        display_element.querySelector('#continue-btn').addEventListener('click', () => {
            ...
            if (stage_num == 1){
                ...
                // CHECK IF trial.number_sticks_selected STICKS WERE SELECTED
                if (advocateSelectedStickValues.length !== trial.number_sticks_selected) {

                    alert(`Please select ${trial.number_sticks_selected} sticks before continuing.`);
                ...
                if (isNumber(estimate)){
                    ...
                    // showAlertAndMaintainFullscreen('Please enter a number for your guess.');
